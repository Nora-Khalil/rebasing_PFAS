...
# %% [markdown]
# ![Screenshot 2025-12-18 at 1.32.59 PM.png](attachment:e1026777-fc41-44ac-8000-eb862c4bfa71.png)
...
# %%
#attempting to get Weber mech sim results to match those of his publication
#instead of adjusting temperature down from 400-750 based on the average difference in set temp versus actual temp in the 800-1100 range, 
#I will just flatline for about 15 cm at the actual set temp from 400-750
...
for temp in temperatures:
   ...
   #append to master dictionary so we can use this is PFR sims
    isothermal_dict[temp] = [distances, temperatures_at_distances, T_profile]
...
for cust_temp in temperatures_to_customize: 
    ...
    isothermal_dict[cust_temp] = [x_values, [final_T_profile(x) for x in x_values], final_T_profile]
...
for temp, [distances, temperatures_at_distances, T_profile] in isothermal_dict.items():
...
# %%
#attempting to get Weber mech sim results to match those of his publication
#will continue to adjust temperature down from 400-750 based on the average difference in set temp versus actual temp in the 800-1100 range, 
#will flatline for about 15 cm at the actual set temp from 400-750
...
for temp in temperatures:
    ...
    #append to master dictionary so we can use this is PFR sims
    isothermal_custT_dict[temp] = [distances, temperatures_at_distances, T_profile]
...
for cust_temp in temperatures_to_customize: 
    ...
    for x in x_values: 
        if x<22.5: 
            y = T_profile_edges(x)
        if x>=22.5 and x<=37.5: 
            y = cust_temp - average_difference - 50 #drop middle section down so it connects to edges
        if x>37.5: 
            y = T_profile_edges(x)
        desired_y_values.append(y)
    ...
    #overwrite with new custom temp profiles 
    isothermal_custT_dict[cust_temp] = [x_values, [final_T_profile(x) for x in x_values], final_T_profile]
...
for temp, [distances, temperatures_at_distances, T_profile] in isothermal_custT_dict.items():
...
#     all_species = ct.Species.listFromFile(mech_path)
#     species = []
...
#                         add_in=False
        ...
        # spec_of_interest = ['C7F14O(323)']
        # should_have = ['CO(8)', 'C6F13']
        # if all(spec in R.equation for spec in spec_of_interest):
        #     if any(spec in R.equation for spec in should_have):
        #         print(f'keeping {str(R.equation)}')
...
species_to_plot = [['HF(38)', 'HF(122)', 'HF(38)', 'HF(4)', 'HF(4)', 'HF'], #HF
                    ['CF2CF2(61)', 'CF2:CF2(151)', None, 'C2F4(5)', 'C2F4(5)','C2F4'], #C2F4
                    ['CO2(13)',  'CO2(13)', 'CO2(13)', 'CO2(6)', 'CO2(6)', 'CO2'], #CO2
                    [None, 'CF3-CF3(162)', None, 'C2F6(7)', 'C2F6(7)', 'C2F6'],   #C2F6, 
                    [None, None, None, 'CO(8)', 'CO(8)',  'CO'], #CO
                    [None, 'CF4(135)', 'CF4(44)', 'CF4(9)', 'CF4(9)', 'CF4'], #CF4
                    [ 'CF2O(49)', 'CF2:O(131)', 'CF2O(49)', 'COF2(10)', 'COF2(10)',  'COF2'],  #COF2
                  ]          
...
for species_of_interest in species_to_plot: 
    ...
    for temp in all_temperatures: 
        ...
        #for spec_of_interest, existing_data, model_version in zip(species_of_interest, master_tuple_for_species, ['after', 'main', 'just_PFAS', 'cpn_maxCarbonatoms_spec_in_core', 'cpn_maxCarbonatoms_seed_spec_in_core','Weber']): 
        for spec_of_interest, existing_data, model_version in zip(species_of_interest, master_tuple_for_species, ['Weber_GPT', 'Weber']): 
            simulate_flag = True
            print(f'simulating {model_version}')
            ...
            if simulate_flag==True: 
                try: 
                    states, gas, sensitivity_data = run_sim(temp, model_version, calculate_sensitivities=False)
                    sens_data_master.append(sensitivity_data)   

                    if model_version=='Weber' or model_version=='Weber_GPT':
...
for species_of_interest in species_to_plot: 
    ...
    for temp in all_temperatures: 
        ...
        #for spec_of_interest, existing_data, model_version in zip(species_of_interest, master_tuple_for_species, ['after', 'main', 'just_PFAS', 'cpn_maxCarbonatoms_spec_in_core', 'cpn_maxCarbonatoms_seed_spec_in_core','Weber']): 
        #model_path = '/projects/westgroup/nora/Code/projects/rebasing_PFAS/models/PFAS/insights_from_Weber/PFOA+Air/fix_reg_spec_in_core/lower_tolerance/forbidden_group/chemkin/copies/copy_chem0226-test.cti'
        #model_path = mech_paths['Weber']
        #for spec_of_interest, existing_data, model_version in zip(species_of_interest, master_tuple_for_species, [model_path]): 
        for spec_of_interest, existing_data, model_version in zip(species_of_interest, master_tuple_for_species, ['Weber']): 
            ...
            if simulate_flag==True: 
                try: 
                    states, gas, sensitivity_data = run_sim_sync_state(temp, model_version, custom_mech_path=False, calculate_sensitivities=False)
                    sens_data_master.append(sensitivity_data)   
...
temps_of_interest = {'HF(4)': None, #[700, 750, 800, 850, 900], 
                    'C2F4(5)': None, #[750], #[600, 650, 700, 750, 800], #C2F4
                    'CO2(6)': None, #[800], #CO2
...
# %%
should_have = ['OH(26)', 'C2F4(5)', 'HF(4)', 'C2F3O(421)']
...
# %% jupyter={"source_hidden": true}
file_path = "./all_mechs_cor.json"
with open(file_path, 'w') as json_file:
    json.dump(master_dictionary_for_concentrations, json_file, indent=4)

# %% jupyter={"source_hidden": true}
file_path = "./cpn_maxCarbonatoms_seed_spec_in_core_forbidden_group.json"
with open(file_path, 'w') as json_file:
    json.dump(master_dictionary_for_concentrations, json_file, indent=4)

# %% jupyter={"source_hidden": true}
file_path = "./master_dictionary_fixing_reg_full_mech.json"

with open(file_path, 'r') as file:
        master_dictionary_fix_reg_full_mech = json.load(file)
